...
class Server(PodSixNet.Server.Server):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.versionNeeded = "0.5"  # The version is a string so it doesn't get changed
        self.games = []
    ...
    def Connected(self, channel, addr):
        ...
        self.playerchannels[self.currentPlayerID].Send({"action": "connecting", "id": self.currentPlayerID,
                                                        "version": self.versionNeeded})
...
class Game:
    ...
    def __repr__(self):
        return "GameNo: " + str(self.gameid) + "\n" + \
               "Cards: " + str(self.cards) + "\n" + \
               "Player0: " + str(self.player0) + "\n" + \
...
class Deck:
    ...
    def __init__(self):
        ...
        self.cardnames = ["frog", "eel", "goose", "horse", "rabbit", "cobra", "rooster", "ox",
                          ...
                          "turtle", "giraffe", "phoenix", "kirin",
                          "iguana", "bear", "rat", "viper", "dog", "otter",
                          "panda", "mouse", "sable", "fox", "sea snake", "tanuki"]
...
class GetInputThread(threading.Thread):

    def __init__(self):
        super(GetInputThread, self).__init__()
        self.isactive = True

    def run(self):

        while self.isactive:
            action = raw_input("Enter Input: ")
            self.handleaction(action)

        print("Quitting game")

    def handleaction(self, action):
        if ("exit" or "quit") in action:
            self.isactive = False
            return

        if ("hello" or "hey") in action.lower():
            print("Hey Jer!")

        if "print" in action.lower():
            print_action = action.replace("print", "")

            if "game" in print_action.lower():
                print_action = action.replace("game", "")
                self.clean_string(action)
                self.print_game(action)

    def clean_string(self, action):
        return action.strip()
